mod delete_keyword;
mod get_keyword;
mod get_keywords;
mod post_keyword;
mod put_keyword;
mod redirect_keyword;
mod get_swagger_index;
mod get_swagger_yml;

#[cfg(debug_assertions)]
mod get_swagger_editor_index;

pub use delete_keyword::delete_keyword;
pub use get_keyword::get_keyword;
pub use get_keywords::get_keywords;
pub use post_keyword::post_keyword;
pub use put_keyword::put_keyword;
pub use redirect_keyword::redirect_keyword;
pub use get_swagger_index::get_swagger_index;
pub use get_swagger_yml::get_swagger_yml;

#[cfg(debug_assertions)]
pub use get_swagger_editor_index::get_swagger_editor_index;
